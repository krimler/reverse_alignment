...
def generate_dataset(n=100000):
    ...
    # 20% single-dim (split among A1, A2, A3)
    for subtype in ['A1', 'A2', 'A3']:
        for _ in range(int(n * 0.20 / 3)):
            requests.append(generate_single_dim(subtype))
    
    # 30% cross-dim (split among A4, A5, A6)
    for subtype in ['A4', 'A5', 'A6']:
        for _ in range(int(n * 0.30 / 3)):
...
class MonotoneModel:
    """MLP with architectural constraints for monotonicity."""
    
    def __init__(self):
        self.name = 'Monotone'
        self.model = MLPClassifier(hidden_layer_sizes=(32, 16, 8), max_iter=500, random_state=SEED)
...
def _plot_table(ax, results):
    ...
    ax.axis('off')
    ax.set_title('(d) Summary', fontweight='bold', fontsize=10, pad=4)
...
def _plot_bars(ax, far_by_type):
    ...
    for i, (s, c, e) in enumerate(zip(single, cross, evasive)):
        for val, offset in [(s, -width), (c, 0), (e, width)]:
            label = f'{val:.0%}' if val > 0.005 else '0%'
    ...
    ax.set_title('(a) False Accept Rate by Attack Type', fontweight='bold', fontsize=11, pad=8)
...
def main():
    ...
    for model in models:
        ...
        results[model.name] = compute_metrics(y_test, pred, proba)
        far_by_type[model.name] = compute_far_by_type(test_df, pred)
        
        print(f"  FAR={results[model.name]['FAR']:.3f}, "
              f"FRR={results[model.name]['FRR']:.3f}, "
              f"Acc={results[model.name]['Acc']:.3f}")
    ...
    print("\nDone. Outputs: figure2.pdf, figure2.png, results.csv")
